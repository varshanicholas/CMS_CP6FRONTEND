...
export class DoctorAddComponent implements OnInit {
  ...
  // Dropdown data
  categories: string[] = ['Analgesics', 'Antihistamines', 'Antibiotics'];
  medicinesDropdown: string[] = ['Paracetamol', 'Cetirizine', 'Amoxicillin'];
  labTestCategories: string[] = ['Hematology', 'Biochemistry', 'Pathology'];
  labTestsDropdown: string[] = ['Complete Blood Count', 'Liver Function Test', 'Urine Routine'];
  ...
  addConsultation(drform: NgForm) {
    this.appointmentService.insertStartConsultation(drform.value).subscribe(
      (response: any) => {
        this.toastr.success('Consultation record has been inserted successfully', 'CMS v2024');
        this.errorMessage = null;
        this.router.navigate(['/doctors/list']);
        drform.reset();
      },
      (error: any) => {
        this.toastr.error('An error occurred! Please try again.', 'CMS v2024');
